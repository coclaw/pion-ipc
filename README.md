...
**Methods**: `pc.create` (accepts optional `settings` for SCTP / ICE / STUN tuning — see [ipc-protocol.md](docs/ipc-protocol.md#pccreate)), `pc.close`, `pc.createOffer`, `pc.createAnswer`, `pc.setRemoteDescription`, `pc.setLocalDescription`, `pc.addIceCandidate`, `pc.restartIce`, `pc.getSctpStats` (SCTP association snapshot — see [ipc-protocol.md](docs/ipc-protocol.md#pcgetsctpstats)), `dc.create`, `dc.send`, `dc.close`, `dc.setBALT`, `dc.getBA`, `ping`
